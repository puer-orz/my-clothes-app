...
function showGameModal(gameId) {
  try {
    ...
    const contentWithBackButton = `
      <div class="sticky top-0 z-50 bg-gray-800 bg-opacity-95 h-14 flex items-center mb-6 border-b border-gray-700 backdrop-blur-sm -mx-8 px-8 -mt-8">
        <button id="back-button" class="px-3 py-1 bg-gray-700 hover:bg-gray-600 text-white rounded-full transition-all duration-300 flex items-center text-sm shadow-lg">
        ...
        case 'three-gardens':
          ...
          if (createGardenBtn && customGardenInput && customGardenResult) {
            createGardenBtn.addEventListener('click', function() {
              ...
              // 预定义词库 (现在从 gardenData 获取，如果未定义则使用空对象)
              const gardenDict = typeof gardenData !== 'undefined' ? gardenData : {};
              ...
              if (existingGarden) {
                ...
                if (matchedKey) {
                  ...
                  customGardenResult.innerHTML = `
                    <div class="bg-red-900 bg-opacity-30 p-4 rounded-lg border border-red-500 border-opacity-50 relative">
                      <button id="close-custom-garden" class="absolute top-2 right-2 text-red-400 hover:text-white transition-colors duration-300">
